...
export default function Homepage() {
  ...
  const takePhoto = async () => {
    if (canvasRef.current && videoRef.current) {
      ...
      if (context) {
        ...
        localStorage.setItem('frameImage', '/footleft1.png'); // Path to your frame image
  ...
  return (
    <main className={styles.main}>
        ...
        />
      <p className={styles.footTextTitle}>ภาพวาดเท้าบนกระดาษข้างซ้าย</p>
      ...
      <div className={styles.camera}>
        <div className={styles["video-wrapper"]}>
          ...
          <div className={styles.overlay} onMouseEnter={handleFootEnter} onMouseLeave={handleFootLeave}>
            <img ref={frameRef} src="/footleft1.png" alt="Frame" className={styles["frame-line"]} />
          </div>
        </div>
        {isFootInFrame && (
