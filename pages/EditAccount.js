...
//         uploadFormData.append("UserId", UserId); // Ensure this is not undefined
...
const EditAccount = () => {
  ...
  useEffect(() => {
    const fetchSessionData = async () => {
      try {
        const res = await fetch("/api/getSession"); // ใช้ API getSession เพื่อดึง userId จาก session
        const data = await res.json();
console.log(data)
        if (res.ok && data.userId) {
          setUserId(data.userId);
        } else {
          setMessage(data.message || "ไม่พบ session ผู้ใช้");
          setIsLoading(false);
        }
      } catch (error) {
        console.error("Error fetching session:", error);
        setMessage("เกิดข้อผิดพลาดในการดึงข้อมูล session");
        setIsLoading(false);
      }
    };
      ...
      if (userId) {
        try {
          const response = await fetch(`/api/user`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId: userId }), // ส่ง userId จาก session
          });
        ...
        setMessage("ไม่พบ UserId ใน session");
        setIsLoading(false);
      }
    };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // ถ้ามีการเลือกภาพโปรไฟล์ใหม่ให้อัปโหลดก่อน
      if (profileImageFile) {
        const uploadFormData = new FormData();
        uploadFormData.append("file", profileImageFile);
        uploadFormData.append("UserId", userId); // ใช้ userId จาก session
        ...
        if (uploadRes.ok && uploadData.success) {
          ...
          setMessage(`เกิดข้อผิดพลาดในการอัปโหลดภาพ: ${uploadData.message}`);
          setIsLoading(false); // หยุดการโหลดเนื่องจากเกิดข้อผิดพลาด
          return;
        }
      }
      ...
      const res = await fetch("/api/updateUser", {
        ...
        body: JSON.stringify({
          userId: userId, // ใช้ userId จาก session
          ...userData,
          profileImage: uploadedImageUrl, // ส่ง URL ของรูปที่อัปโหลด
        }),
      });
      ...
      setTimeout(() => {
        setIsLoading(false); // ยกเลิกสถานะ loading หลังจากส่งข้อมูลเสร็จ
      }, 2000); // แสดง loading อย่างน้อย 2 วินาที
    }
  };
